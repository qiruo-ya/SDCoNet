...
class BlockBody(nn.Module):
    ...
    def forward(self, x):
        ...
        scalezero = self.asff_scalezero1(x0, F.interpolate(self.upsample_scaleone1_2(x1), (x0.shape[2], x0.shape[3])))
        scaleone = self.asff_scaleone1(F.interpolate(self.downsample_scalezero1_2(x0), (x1.shape[2], x1.shape[3])), x1)

        x0 = self.blocks_scalezero2(scalezero)
        x1 = self.blocks_scaleone2(scaleone)

        scalezero = self.asff_scalezero2(x0, F.interpolate(self.upsample_scaleone2_2(x1), (x0.shape[2], x0.shape[3])),
                                         F.interpolate(self.upsample_scaletwo2_4(x2), (x0.shape[2], x0.shape[3])))
        scaleone = self.asff_scaleone2(F.interpolate(self.downsample_scalezero2_2(x0), (x1.shape[2], x1.shape[3])), x1,
                                       F.interpolate(self.upsample_scaletwo2_2(x2), (x1.shape[2], x1.shape[3])))
        scaletwo = self.asff_scaletwo2(F.interpolate(self.downsample_scalezero2_4(x0), (x2.shape[2], x2.shape[3])),
                                       F.interpolate(self.downsample_scaleone2_2(x1), (x2.shape[2], x2.shape[3])), x2)

        x0 = self.blocks_scalezero3(scalezero)
        x1 = self.blocks_scaleone3(scaleone)
        x2 = self.blocks_scaletwo3(scaletwo)

        scalezero = self.asff_scalezero3(x0, F.interpolate(self.upsample_scaleone3_2(x1), (x0.shape[2], x0.shape[3])),
                                         F.interpolate(self.upsample_scaletwo3_4(x2), (x0.shape[2], x0.shape[3])),
                                         F.interpolate(self.upsample_scalethree3_8(x3), (x0.shape[2], x0.shape[3])))
        scaleone = self.asff_scaleone3(F.interpolate(self.downsample_scalezero3_2(x0), (x1.shape[2], x1.shape[3])), x1,
                                       F.interpolate(self.upsample_scaletwo3_2(x2), (x1.shape[2], x1.shape[3])),
                                       F.interpolate(self.upsample_scalethree3_4(x3), (x1.shape[2], x1.shape[3])))
        scaletwo = self.asff_scaletwo3(F.interpolate(self.downsample_scalezero3_4(x0), (x2.shape[2], x2.shape[3])),
                                       F.interpolate(self.downsample_scaleone3_2(x1), (x2.shape[2], x2.shape[3])), x2,
                                       F.interpolate(self.upsample_scalethree3_2(x3), (x2.shape[2], x2.shape[3])))
        scalethree = self.asff_scalethree3(F.interpolate(self.downsample_scalezero3_8(x0), (x3.shape[2], x3.shape[3])),
                                           F.interpolate(self.downsample_scaleone3_4(x1), (x3.shape[2], x3.shape[3])),
                                           F.interpolate(self.downsample_scaletwo3_2(x2), (x3.shape[2], x3.shape[3])),
                                           x3)
