...
class SA_DINO_BG(SA_DeformableDETR_BG):
    ...
    def __init__(self, *args, dn_cfg: OptConfigType = None,salience_criterion=None, **kwargs) -> None:
        ...
        self.salience_criterion = SalienceCriterion(alpha=0.25, gamma=2.0)
    ...
    def init_weights(self) -> None:
        ...
        # nn.init.constant_(self.bbox_head.cls_branches[-1], bias_value)
        # # initiailize encoder regression layers
        # nn.init.constant_(self.bbox_head.reg_branches[-1].weight, 0.0)
        # nn.init.constant_(self.bbox_head.reg_branches[-1].bias, 0.0)
        if isinstance(self.bbox_head.cls_branches[-1], nn.Linear):
            nn.init.constant_(self.bbox_head.cls_branches[-1].bias, bias_value)
            # Initialize regression branch (for bounding box predictions)
        if isinstance(self.bbox_head.reg_branches[-1], nn.Linear):
            nn.init.constant_(self.bbox_head.reg_branches[-1].weight, 0.0)
            nn.init.constant_(self.bbox_head.reg_branches[-1].bias, 0.0)
    ...
    def loss(self, batch_inputs, batch_data_samples):
        ...
        # 提取特征
        img_feats = self.extract_feat(low_res_inputs)
        # img_feats = self.extract_feat(batch_inputs)

        # 获取 Transformer 输出
        head_inputs_dict = self.forward_transformer(img_feats, batch_data_samples)

        # 移除 head_inputs_dict 中的 'memory' 键，传入 bbox_head.loss
        ...
        # 获取 foreground_score 和 multi-level 特征
        foreground_score = head_inputs_dict.get('memory', None)
        mlvl_feats = self.last_feat  # 从 forward_transformer 中获取多层特征

        if foreground_score is not None and mlvl_feats is not None:
            # 计算 feature_strides，注意此处确保使用正确的图像尺寸
            ...
            for sample in batch_data_samples:
                ...
                # 通常是 xyxy 格式，需要手动转为 cxcywh 格式并归一化
